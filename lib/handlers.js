...
handlers._users.get = function(data, callback) {
  ...
  if(phone){
    ...
    // Verificar se o token é válido para o número de telefone
    handlers._tokens.verifyToken(token,phone,(tokenIsValid)=>{
      if(tokenIsValid){
        ...
        callback(403,{'Erro':'O token não está no header, ou o token é inválido'})
      }
    })
  } else {
...
// Verifica se um dado token id é válido para um determinado user
handlers._tokens.verifyToken = function(id,phone,callback){
  // Procurar pelo token
  _data.read('tokens',id,(err,tokenData)=>{
    if(!err && tokenData){
      // Checar se o token pertence ao usuário e não expirou
...
// Containers para todos os métodos de Checks
helpers._checks = {}
// Not found handler
